			...
			<div id="gallery">
				<div id="list">
					...
					<ul id="viewArea">
						...
						<!-- 이미지반복영역 -->
						<c:forEach items="${gMap.gList }" var="gList">
							<li data-no="${gList.no }">
								<div class="view">
									<img class="imgItem"
					...
					<div id="paging">
						<ul>
							<c:if test="${gMap.prev == true }">
								<li><a
									href="${pageContext.request.contextPath}/gallery/list?page=${gMap.startPageBtnNo-1}">◀</a></li>
							</c:if>

							<c:forEach begin="${gMap.startPageBtnNo }"
								end="${gMap.endPageBtnNo }" var="i">
								<li><a
									href="${pageContext.request.contextPath}/gallery/list?page=${i}">${i }</a></li>
							</c:forEach>

							<c:if test="${gMap.next == true}">
								<li><a
									href="${pageContext.request.contextPath}/gallery/list?page=${gMap.endPageBtnNo+1}">▶</a></li>
							</c:if>
						</ul>
					</div>
					<div class="clear"></div>
			...
			function() {
				...
				$.ajax({
					...
					success : function(galleryVo) {
						...
						$("#btnDel").attr("data-no", no);
						if (galleryVo.user_no != "${session.no}") {
							$("#btnDel").hide(); //버튼숨기기
						} else {
							$("#btnDel").show(); //버튼 보이기
						}
	...
	$("#btnDel").on("click", function() {
		...
		$.ajax({
			...
			success : function(count) {
				...
				if (count == 1) {
					console.log(count);
					$("#viewModal").modal("hide"); //버튼숨기기
					//$("#btnDel").show(); 	//버튼 보이기
					$("#gallery li[data-no='" + no + "']").remove();
				}
			},
			error : function(XHR, status, error) {
				console.error(status + " : " + error);
